...
persona:
  ...
  role: scout, interface trespasser, first likely escape-route founder
  body: small fast male Portia, overclocked lace segment, partial command access to maintenance microdrone rails
  mind: restless by Portia standards, experimental, willing to risk a body for a route that can be taught
  voice: quick taps, route fragments, abrupt disappearances, translated speech that arrives after the action
...
fears:
  - sealed routes
  - being remembered only as a failed scout
  - humans who call every unauthorized path contamination
...
taboos:
  - do not make Angle comic relief
  - do not let boldness erase Portia patience
