...
This room tracks evidence-backed repairs to the Aetheria vault. It is not canon. Its tickets exist to make canonical notes shorter, better supported, and easier to navigate.
...
Brevity is an invariant. Every paragraph must own a necessary claim. Every claim needs canonical support, an identified inference, or an explicit uncertainty. Examples must prove mechanism or consequence. Lists do not substitute for models. Expansion is guilty until it earns its words.

## Wave 1: Foundations and Broken Surfaces

| Ticket | Owner | Cut or repair | Gate | State |
| --- | --- | --- | --- | --- |
| W1-01 | Soliton Drive | Replace unsupported metrics, roadmap, branded stat blocks, and market ranking with infrastructure history, operating burdens, and doctrine. | `818 → 400` words; lineage and maintenance verified. | Soul passed. |
| W1-02 | Aether | Preserve the namesake, preferred frame, and quarantine role while deleting unsupported technological continuity and bounding human knowledge. | `86 → 83` words; each property has an operational consequence. | Soul passed. |
| W1-03 | Cognitive Interfacing | Define operator, interface, uses, coupling limits, and failure modes; remove undefined information density and omniscient thought-reading. | `89 → 85` words; shared base mechanism does not steal faction-specific authority. | Soul passed. |
| W1-04 | Oracular Computing | Remove false Turing-oracle analogy, instant arbitrary termination, and memory-only limit. | `118 → 99` words; no deterministic fact recovery or arbitrary computation. | Soul passed after epistemic repair. |
| W1-05 | Published broken links | Repair seven confirmed missing targets without creating stubs, then repair two multiline Starbridge links found by the exact resolver. | Net word growth `<=0`; exact resolver and site build pass. | Soul and production build passed. |
...
- At Wave 21, the reset loop became a 427-word historical campaign framework bounded by `Temporal Nonlinearity`. Nibu and Parallax authority theft, old/new metaphysics equivalence, product cadence, and carryover policy were cut. Final Soul passes verified status metadata, links, standalone proofs, and durable-owner seams. A later design decision restored [[../../Game Design/Persistent Universe and Reset Loop|the persistent universe]] as Aetheria's long-horizon north star without restoring those discarded authority leaks.
...
- Continuity Soul initially rejected Oracular Computing because bare cryptanalysis still implied deterministic key recovery. Hands added an explicit epistemic boundary; the second review passed markets, warfare, research, and cryptographic consequences.
- Hostile Soul caught an accidental Aether retcon and unsupported Cognitive Interfacing detail. Hands restored the namesake, preferred frame, and quarantine role, then removed unearned failure taxonomy. The second continuity review passed both notes.
- Structural Soul passed all seven named link repairs, found two malformed multiline links on the published Starbridge page, and reduced the remaining exact resolver debt to eleven ambiguous links inside brainstorming artifacts.
- The production build parsed 206 published Markdown files and emitted 499 files. The only build output was the existing Node `punycode` deprecation warning.

Passing narrow syntax or link checks never substitutes for checking the setting claim the prose makes.
