# Decision Log

| Decision | Status | Rationale | Rejected alternative |
| --- | --- | --- | --- |
| No word-count target or ceiling. | Operator-locked | Length follows completed dramatic work. | Fixed-length production constraints. |
| Twenty-two chapters are provisional architecture. | Accepted | Scene and prose evidence may split or collapse them. | Chapter count as production quota. |
| Full fourteen-pass Foundry run. | Accepted | Reverse chronology, distributed personhood, and ensemble causality justify inspectable state. | Retroactive pass completion. |
| The Unseating spans the Harrowglass Route Sphere. | Accepted | Supports eleven governments and successor politics without rewriting all Elysium. | Civilization-wide rupture or unnamed scale. |
| Meridian POV normally rotates among the five cadets. | Accepted | Keeps the manufactured household central. | Unbounded ensemble omniscience. |
| AFTER receives bounded close third after it can claim personhood. | Accepted | Certifies sixth-person status without granting historical omniscience. | Never entering AFTER or using it as an exposition camera. |
| Calyx concentrates on Mara and Ysra. | Accepted | Gives reverse history stable emotional anchors. | Automatic five-way historical rotation. |
| Escape uses the five damaged component Figures. | Accepted | Preserves bodily autonomy and makes shelter a live material problem. | Convenient third-party home or permanent combined body. |
| One final machine-person reply is dramatized; two signals remain latent. | Accepted | Gives the ending one relationship rather than three sequel pitches. | Three closing introductions or only one total signal. |
| Story-local invention remains outside setting canon during drafting. | Accepted | The manuscript must prove what history it needs. | Silent promotion from planning. |
| Academy Figures are single-occupant in flight with assigned crews based in bay or tender. | Accepted | Preserves Daedal crew labor without carrying invisible unconsenting crews into Concordance. | Five unexplained onboard crews or labor-free machines. |
| Meridian uses a rotating school drum around a non-rotating flight truss. | Accepted | Makes school routine and zero-g flight thresholds legible. | Scene-dependent campus geometry. |
| Senn, Tem, and Iri receive bounded class/family provenance. | Accepted | Their dependencies and exits must be as material as Omi and Lio's. | Generic academy-native backgrounds. |
| The ending route is Morrow Quay pallet refusal to Saint Varo. | Accepted | Honors local refusal and ends at temporary infrastructure rather than safety. | Convenient sanctuary, forced entry, or fade-out in open space. |
| Professional futures organize the academy movement. | Accepted | Plastics and Lifers offer attractive, competent adult lives whose fixed roles divide the household and make resistance relationally costly. | Conspiracy investigation as the academy's sole engine. |
| Candidate prose remains noncanonical until manuscript drafting. | Accepted | Pass 5 tests causal spine, social texture, and voice without smuggling audition language into the final story. | Promoting a generated candidate wholesale. |
| The assembly rewrite uses the minimal candidate's spine. | Accepted | It preserves mechanical intelligibility, emotional escalation, and a sustainable voice; bounded social facts from the full-room test remain available. | Scene-level collage or the full-room exposition load. |
| A reveal must change material or relational pressure. | Accepted | Knowledge-only turns produce dossier pacing and conceal causal weakness. | Revelation as an independent plot unit. |
| Harrowglass political proof survives outside Eda's private archive. | Accepted | Public petitions, minutes, Sol's agreements, traffic records, and a deletion manifest establish guardianship pressure without undoing her protection of petitioners. | Recovering the burned archive. |
| Vesper access is bounded to custody safety review. | Accepted | Executive exclusion and involuntary pattern activation can become bodily evidence without breaching Vey's clearance boundary or spoiling Necro. | Generic classified-file access. |
| AFTER's Calyx continuity begins as a credible claim. | Accepted | Repeated embodied behavior supports relationship before recovered history strengthens provenance. | Instant documentary certainty or records as the source of personhood. |
| The provisional architecture contains twenty-eight chapters. | Accepted | Distinct dramatic decisions no longer share overloaded containers; drafting may still split or collapse them. | Twenty-two inherited chapters as quota or chapter count as story length. |
| Harrowglass names AFTER to the reader. | Accepted | Mara's close POV honestly knows the sixth participant; Meridian's later ladder tests survival and recognition rather than original existence. | Coyly hiding AFTER's name from Mara or giving Meridian unearned knowledge. |
| Movement III uses five chapters and fourteen physical scene turns. | Accepted | Chapter containers preserve momentum while scene transactions retain distinct objectives, owners, and costs. | One continuous climax or lore-driven segmentation. |
