...
Bulk kernel runs populate balance and decision-training datasets from native instance IDs and provenance, allowing results to be grouped by blueprint, manufacturer, recipe input, quality, doctrine, or condition without confusing those categories. The performance target is many times faster than realtime with hundreds of concurrent battles across the local galaxy. This is infrastructure for Aetheria's community-warfare and persistent strategic layers, not only the scoped [[Aetheria Starbridge]] mode. Profits Rising corporation gameplay consumes this machinery through logistics, procurement, production, and strategic command during seasonal warfare.
