# Selected Counter Traces

These packets supplement [[../Tactical Traces]] for the campaigns accepted into [[../Failure Ladder]]. They are narrative causality records, not executable simulation output.

## C1: Refundable War Warning

| Field | Accepted state |
| --- | --- |
| Objective | Clear a heterogeneous screen, preserve the target, and recover the full Courtesy wing. |
...
## C2: Prior Absence Abort

| Field | Accepted state |
| --- | --- |
| Objective | Mature attachment and surrender geometry without waking the full ship. |
...
## C3: False Sovereignty

| Field | Accepted state |
| --- | --- |
| Objective | Capture a commercial hull without endangering passengers or dependency cargo. |
...
| Field | Accepted state |
| --- | --- |
| Objective | Recover a corrected Courtesy flight after contested screen clearing. |
| False belief | Known recovery vulnerability has been mastered by reserve, relocation, shutters, armor, and dedicated PDC. |
| Commitment | A damaged friendly and an occluded loiterer enter minimum safe PDC geometry while a second terminal body remains committed. |
| Decisive effect | Jo's correct defensive shot prevents entry but jams an open shutter and opens propellant transfer; the second loiterer strikes the exposed service neighborhood and internal stores propagate the loss. |
| Human decision | Luce authorizes destruction of his own drone; Dena clears Deck; Samira evacuates; Pal severs embodiment. |
| Competent alternative | Abandon the damaged drone earlier, sacrificing its cognition and records; this reduces risk but was not obviously mandatory before occlusion resolves. |
| Residue | Hull One scuttled, all people rescued, Pal and Twelvefold embodiment/cache losses, family leverage rises. |
| Status | Topology and stored-energy propagation Established broadly; fragment damage and occluded terminal approach Provisional but not knife-edge. |

## C5: The Last Plausible Ship

| Field | Accepted state |
| --- | --- |
| Objective | Use ferries to divide attention and attach capture systems before the convoy reaches protection. |
...
| Field | Accepted state |
| --- | --- |
| Objective | Precisely disable resistance before conventional ballistic exchange matures. |
...
## C7A: Evident Intent Opening Bet

| Field | Accepted state |
| --- | --- |
| Objective | Protect contracted traffic while retaining capture opportunity. |
| False belief | A ship able to dominate one engagement volume can force the opponent's important action to occur there. |
| Commitment | Luce orders an initial burn toward a convincing decoy threat. |
| Decisive effect | Another operator matures the real objective outside the Hot ship's reachable volume. |
| Human decision | Samira refuses further pursuit; Luce accepts, reducing loss but not recovering the contract. |
| Competent alternative | Hold the protected volume and surrender the tempting capture from the beginning. |
| Residue | Hull survives; ammunition and contract costs constrain the continuing salvage phase; Samira loses a credible exit through hostile interpretation. |
| Status | Objective clock and spatial commitment Established. |

## C7B: Parliament Rescue Failure

| Field | Accepted state |
| --- | --- |
| Objective | Capture one carrier while preserving parent and rescue capacity. |
| False belief | Heterogeneous role ambiguity forces every competent defender to classify individual contacts. |
| Commitment | Cloud disperses beyond reliable high-bandwidth control while rescue shepherds remain withheld. |
| Decisive effect | Consequence-volume fire and link partition attack shared space and infrastructure rather than roles; capture geometry collapses. |
| Human decision | Sable delegates locally; Twelvefold records split; Luce preserves rescue reserve; Dena halts recovery; Jo protects parent. |
| Competent alternative | Spend one shepherd as a relay and improve capture odds at the cost of casualty reserve. |
| Residue | Prize escapes, blister damaged, Soren and Meera Tann rescued from a detached recovery tender by withheld craft, Wake credit closes. |
| Status | Capability envelopes Established; consequence-volume policy and protected reserve Provisional and recommended for simulation promotion. |

## Robustness Rule

Pass 7 may refine who knew what and when. It may not make a loss depend on:

- exact percentage superiority;
- an unseeded weapon or target weakness;
- a human approving a millisecond terminal action;
- adversarial cognition compelling behavior;
- absent hardware appearing through a role change;
- officers silently overriding established domain authority;
- an opponent continuing a fight after its rational objective has vanished merely to deliver the planned ending.
