...
export type AetheriaSidebarGroup = {
  title: string
  links: AetheriaSidebarLink[]
}

export type AetheriaSidebarData = {
  title: string
  slug: FullSlug
  tagline?: string
  summary?: string
  groups: AetheriaSidebarGroup[]
}

export type AetheriaPageContext = {
  headerTagline?: string
  sidebar?: AetheriaSidebarData
}
...
function topLevelElements(root: Root) {
  return root.children
    ...
    .filter((entry): entry is { node: Element; index: number } => isElement(entry.node))
}
...
export function normalizeAetheriaSlug(slug: string) {
  if (slug === "index") {
    return slug
  }
...
export function isSidebarLinkActive(currentSlug: FullSlug, targetSlug: FullSlug) {
  const normalizedCurrent = normalizeAetheriaSlug(currentSlug)
  const normalizedTarget = normalizeAetheriaSlug(targetSlug)
...
function extractOverviewGroups(root: Root, sourceSlug: FullSlug): AetheriaSidebarGroup[] {
  const groups: AetheriaSidebarGroup[] = []
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    const links: AetheriaSidebarLink[] = []
...
function overviewCandidates(currentSlug: FullSlug, includeCurrent = true) {
  ...
  const normalized = normalizeAetheriaSlug(currentSlug)
...
export function findNearestOverviewNote(currentSlug: FullSlug, allFiles: QuartzPluginData[]) {
  ...
  for (const candidate of overviewCandidates(currentSlug)) {
...
export function findSidebarOverviewNote(currentSlug: FullSlug, allFiles: QuartzPluginData[]) {
  ...
  const includeCurrent = !isOverviewSlug(currentSlug)
...
function extractOverviewData(file: QuartzPluginData): AetheriaSidebarData | undefined {
  ...
  return {
    title: file.frontmatter?.title ?? "Aetheria",
    slug: file.slug,
...
export function buildAetheriaPageContext(
  currentRoot: Root,
